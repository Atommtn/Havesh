...
public class SignalRRegisterUserGrain : HaveshManagerGrainBase, ISignalRRegisterUserGrain
{
    ...
    {
        var user = await _grainFactory.GetGrain<IHaveshGrain<User>>( userId, GrainBranchKey).Get();
        if (user != null)
        {
    ...
    public async Task UnregisterUser(int userId, string connectionId)
    {
        ...
        var userGrain = _grainFactory.GetGrain<IHaveshGrain<User>>( userId, GrainBranchKey);
