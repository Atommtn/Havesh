...
public class DataProviderService : IAsyncDisposable , IDisposable
{
    ...
    /// <summary>
    /// فقط موقع ساخت ترم جدید (isNewTerm) صدا زده می‌شه: ترمی که EndDate‌اش نزدیک‌ترین
    /// (و قبل/هم‌زمان با) StartDate ترم جدیده رو پیدا می‌کند و NextTermFk آن ترم قبلی
    /// را به ترم جدید وصل می‌کند، و LastTermFk ترم جدید را به آن ترم قبلی.
    /// </summary>
    private void LinkTermToPreviousTerm(ShokouhPardisTermClass newTerm)
    {
        var previousTerm = DbContext.ShokouhPardisTermClasses
            .Where(x => x.Id != newTerm.Id && x.EndDate <= newTerm.StartDate)
            .OrderByDescending(x => x.EndDate)
            .FirstOrDefault();

        if (previousTerm is null) return;

        previousTerm.NextTermFk = newTerm.Id;
        newTerm.LastTermFk = previousTerm.Id;

        DbContext.ShokouhPardisTermClasses.Update(previousTerm);
        DbContext.ShokouhPardisTermClasses.Update(newTerm);
        SaveAll();
    }

    /// <summary>
    /// همیشه بعد از ذخیره صدا زده می‌شه (چه لینک خودکار موقع ساخت، چه اصلاح دستی موقع ویرایش)
    /// تا NextTermFk/LastTermFk دوطرفه باقی بمونه:
    /// - اگه LastTermFk ترم جاری به ترمی اشاره می‌کند، NextTermFk آن ترم باید همین ترم باشد.
    /// - اگه NextTermFk ترم جاری به ترمی اشاره می‌کند، LastTermFk آن ترم باید همین ترم باشد.
    /// - هر ترم قدیمی‌ای که قبلاً به این ترم اشاره می‌کرد ولی الان دیگه نه، لینکش پاک می‌شه.
    /// </summary>
    private void SyncReciprocalTermLinks(ShokouhPardisTermClass term)
    {
        var changed = false;

        var stalePrevLinks = DbContext.ShokouhPardisTermClasses
            .Where(x => x.NextTermFk == term.Id && x.Id != term.LastTermFk)
            .ToList();
        foreach (var t in stalePrevLinks)
        {
            t.NextTermFk = null;
            changed = true;
        }

        var staleNextLinks = DbContext.ShokouhPardisTermClasses
            .Where(x => x.LastTermFk == term.Id && x.Id != term.NextTermFk)
            .ToList();
        foreach (var t in staleNextLinks)
        {
            t.LastTermFk = null;
            changed = true;
        }

        if (term.LastTermFk is int lastId)
        {
            var prev = DbContext.ShokouhPardisTermClasses.FirstOrDefault(x => x.Id == lastId);
            if (prev is not null && prev.NextTermFk != term.Id)
            {
                prev.NextTermFk = term.Id;
                changed = true;
            }
        }

        if (term.NextTermFk is int nextId)
        {
            var next = DbContext.ShokouhPardisTermClasses.FirstOrDefault(x => x.Id == nextId);
            if (next is not null && next.LastTermFk != term.Id)
            {
                next.LastTermFk = term.Id;
                changed = true;
            }
        }
